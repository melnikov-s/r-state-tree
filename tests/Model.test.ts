...
describe("model state", () => {
	...
	class MCB extends Model {
		...
		@state prop = 0;
		ignored = 0;
		@child(MCA) mcas: MCA[] = [MCA.create(), MCA.create()];
	...
	class M extends Model {
		...
		ignored = 0;
		@child(MCA) mcas: MCA[] = [];
		@child(MCB) mcbs: MCB[] = [MCB.create(), MCB.create()];
	...
	test("snapshot ids get reconciled for child models", () => {
		...
		class M extends Model {
			@child(MC) mcs = [MC.create({ id: 0 }), MC.create({ id: 1 })];
		}
	...
	test("will throw if reconciled child id exists on another parent", () => {
		...
		class M extends Model {
			@child(MC) mcs: MC[] = [MC.create({ id: 0 }), MC.create({ id: 1 })];
		}

		class MP extends Model {
			@child(M) ms: M[] = [M.create(), M.create()];
		}
	...
	test("will throw if reconciled child id exists on same parent but differnt child models", () => {
		...
		class M extends Model {
			@child(MC) ms1: MC[] = [MC.create({ id: 0 }), MC.create({ id: 1 })];
			@child(MC) ms2: MC[] = [MC.create({ id: 2 }), MC.create({ id: 3 })];
		}
	...
	test("can add and remove model with an id", () => {
		...
		class M extends Model {
			@child(MC) ms: MC[] = [MC.create({ id: 0 }), MC.create({ id: 1 })];
			act() {
	...
	test("changing referenced models trigger a snapshot change", () => {
		...
		const mca = MC.create();
		const mcb = MC.create();
		const m = M.create({ mca: mca, mcb: mcb, mcrs: [mca], mcr: mca });
		onSnapshot(m, () => {
			count++;
		});
		expect(count).toBe(0);
	...
	describe("Snapshot diffs", () => {
		test("onSnapshotDiff applying undo/redo", () => {
			...
			class M extends Model {
				...
				obs = 0;
				@child(MC) mc: MC = MC.create();
				@child(MC) mcs: MC[] = [MC.create(), MC.create({ prop: 1 })];
...
describe("model context", () => {
	...
	test("context with array of children", () => {
		...
		class Parent extends Model {
			...
			@child cs = [Child.create(), Child.create()];
		}
	...
	test("context in detached array element uses default", () => {
		...
		class Parent extends Model {
			...
			@child cs: Child[] = [Child.create({ id: 0 }), Child.create({ id: 1 })];
